The README below is cloned from [the official docker repo for couchbase](https://github.com/couchbase/docker/blob/master/enterprise/couchbase-server/4.0.0-beta/README.md).

Note: for a fast automated setup for localhost dev (rather than the usual web wizard), start a bash shell or run docker with this:

    docker run /usr/local/bin/couchbase-setup.sh
